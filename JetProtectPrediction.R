...
#Applying Naive Bayes(69.46)

NaiveBayesModel<-naiveBayes(as.factor(JetProtect) ~.,data=train)

NBPrediction<-predict(NaiveBayesModel,test)

NB<-table(NBPrediction,test$JetProtect)
confusionMatrix(NB)


#Naive Bayes(k fold cross validation)(76.39)

train_control <- trainControl(method="repeatedcv", number=30,repeats = 3)

#x<-head(train,10000)

NaiveBayesCVmodel <- train(as.factor(JetProtect) ~.,data=train, 
                         trControl=train_control, method="nb")

NBCVPrediction<-predict(NaiveBayesCVmodel,test)

NBCV<-table(NBCVPrediction,test$JetProtect)
confusionMatrix(NBCV)
...
#Logistic regression(k fold cross validation)(76.35)

train_control <- trainControl(method="repeatedcv", number=30,repeats = 3)

#x<-head(train,10000)

LogisticRegressionmodel <- train(as.factor(JetProtect) ~.,
                                   data=train,trControl=train_control, method="glm")

LRPrediction<-predict(LogisticRegressionmodel,test)

LR<-table(LRPrediction,test$JetProtect)
confusionMatrix(LR)


#SVM(k fold cross validation)(76.41)

#x<-head(train,20000)

train_control <- trainControl(method="repeatedcv", number=30,repeats = 3)

SVMModel<-svm(as.factor(JetProtect) ~IsCorporate+TripRT+
...
SVM<-table(SVMPrediction,test$JetProtect)
confusionMatrix(SVM)

#Decision Tree(k fold cross validation)(76.41)
...
DecisionTreemodel <- train(as.factor(JetProtect) ~IsCorporate+TripRT+
                             SourceMOBSITE+DeviceAndroid+DeviceApple+
                             DeviceOthers+X8to14days+X15to21days+X22to30days+
                             X31to60days+X61to90days+Above90days+RevenuePerPax+Night+
                             SEM+SEO+Metasearch+Affiliates+SME+Retargetting+
...
DT<-table(DTPrediction,test$JetProtect)
confusionMatrix(DT)


IsCorporate+TripOW+TripRT+SourceWEB+
SourceMOBSITE+DeviceDesktop+DeviceAndroid+DeviceApple+
 DeviceOthers+X0to7.days+X8to14days+X15to21days+X22to30days+
X31to60days+X61to90days+Above90days+RevenuePerPax+Day+Night+
Direct+SEM+SEO+Metasearch+Affiliates+SME+Retargetting+
Newsletter+Social+Male+Female+Child+Couple+CoupleWithChild+
SingleMale+SingleFemale+SingleMaleWithChild+
SingleFemaleWithChild+MaleWithChild+FemaleWithChild
